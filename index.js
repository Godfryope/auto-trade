...
async function updateSolanaBalance(telegramId) {
  try {
    ...
    console.log(`📊 The Solana balance for wallet ${walletAddress} is: ${Number(solBalance).toFixed(9)} SOL`);
    return Number(solBalance).toFixed(9); // Ensure solBalance is a number before calling toFixed
  } catch (err) {
